...
double var(double mean,char *str);
...
//Defining the function for calculating the variance
double var(double Mean,char *str){
  int i=0;
  FILE *fp;
  double x, temp=0.0,var = 0.0;
  ...
  while(fscanf(fp,"%lf",&x)!=EOF)
  {
    //Count numbers in file
    i=i+1;
    //Squaring the difference of mean and sample number in file
    temp += (x-Mean)*(x-Mean);
